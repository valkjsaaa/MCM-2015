...
\newcommand{\pagetop}{Team \# 39984} %\pagetop to cite it
...
Where $k$ is the amount of money spent on sending the medications for an individual traveling through one unit of distance, a constant parameter that we neither know nor care in this experiment actually (take $S_i$ as an example); $Cost$ is the total cost of building the center in a chosen place; $Dist$ is the distance between the medical center ($S_i$) and any location requesting for medical aids (take $S_j$ for instance); $nCases$ is the amount of medications required by the corresponding location (also $S_j$, in other words).
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=5in]{"Guinea Vaccine".pdf}}
\caption{Vaccine (Guinea)}
\label{GuineaVaccine}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=5in]{"Liberia Vaccine".pdf}}
\caption{Vaccine (Liberia)}
\label{LiberiaVaccine}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=5in]{"Sierra Leone Vaccine".pdf}}
